...
class GulpIO:
    ...
    @staticmethod
    def cif_structure_lines(cifd,cell_flg=True,frac_flg=True,connect=True):
        a,b,c,alpha,beta,gamma=float(cifd["_cell_length_a"]),\
                               float(cifd["_cell_length_b"]),\
                               float(cifd["_cell_length_c"]),\
                               float(cifd["_cell_angle_alpha"]),\
                               float(cifd["_cell_angle_beta"]),\
                               float(cifd["_cell_angle_gamma"])
        bond_type_map={"A":"resonant",
                       "S":"single",
                       "D":"double",
                       "T":"triple"
                       }
        atom_site_label=cifd["_atom_site_label"]
        site_type_symbol=cifd["_atom_site_type_symbol"]
        fx=cifd["_atom_site_fract_x"]
        fy=cifd["_atom_site_fract_y"]
        fz=cifd["_atom_site_fract_z"]
        label1=cifd["_geom_bond_atom_site_label_1"]
        label2=cifd["_geom_bond_atom_site_label_2"]
        geom_sym=cifd["_geom_bond_site_symmetry_2"]
        bond_type=cifd["_ccdc_geom_bond_type"] 
        gin = ""
        if cell_flg:
            gin += "cell\n"
        ...
        if frac_flg:
            ...
            gin += "cart\n"
            coord_attr = "coords"
        for symbol, x, y, z in zip(site_type_symbol,fx,fy,fz):
            coord = [i for i in (x,y,z)]
            core_site_desc = symbol + " core " + " ".join(coord) + "\n"
            gin += core_site_desc

        if connect:
           gin +='\n'
           for l1,l2,sym,bty in zip(label1,label2,geom_sym,bond_type):
               idx_1=atom_site_label.index(l1)+1
               idx_2=atom_site_label.index(l2)+1
               if sym==".":
                  img=[0,0,0]
               else: 
                  img = np.asarray(list(map(int, sym.split('_')[1]))) - 5
               con="connect %d %d %s %d %d %d\n"%(idx_1,idx_2,
                                               bond_type_map[bty],
                                               img[0],img[1],img[2])
               gin +=con
           gin +='\n'
    ...
    def structure_lines(structure, cell_flg=True, frac_flg=True,
        ...
        for site in structure.sites:
            coord = ["%.8f"%(i) for i in getattr(site, coord_attr)]
            specie = site.specie
